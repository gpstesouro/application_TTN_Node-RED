...
> __Observações: (i)__ TTN utiliza `netID` 000013 e `tenantID` (locatário): ttn (V3) and ttnv2 (V2); __(ii)__ tenha paciência ao aguardar as atualizações: o status "offline" demora uns 10 minutos e "online" leva cerca de 1 minuto para ser reportado pela API; __(iii)__ `rxRate` e `txRate` são taxas médias de tráfego dos últimos 6 minutos; e __(iv)__ ainda estou pensando nisto (talvez a futura v1.1), parece viável implementar um indicativo de SLA (Acordo de Nível de Serviço) por gateway.
